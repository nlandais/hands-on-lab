...
profiles = []
File.open(File.expand_path('~/.aws/config'), 'r') do |f|
  f.each_line do |l|
...
# search above doesn't seem to work - hardcode to use default profile for now
profiles = ['default']
...
data = profiles.map do |account|
  ...
  regions.map do |region|
    ...
    JSON.parse(azs_json)['AvailabilityZones'].map do |tuple|
      tuple[:name] = "#{tuple['RegionName']}"
      tuple[:sortkey] = "#{tuple['ZoneName']}"
      tuple
    end
  end.flatten
